...
- [multi-session-lock-helper-plan.md](multi-session-lock-helper-plan.md) — `Plan Review` —
  first implementation slice of ADR 0014: loom's **3rd** executable component — a POSIX-sh
  ...
  **not** a hook (no `hooks.json`). Subcommands: `lock-acquire` / `lock-release` /
  `lock-holder`; `claim` / `renew` / `release-claim` / `reclaim` / `list-claims`;
